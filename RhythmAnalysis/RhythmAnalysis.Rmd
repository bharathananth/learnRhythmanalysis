...
We need to correct to multiple-testing. 

Multiple testing drives the number of false positive up and must be controlled. 
...
To learn about interpreting p-value distributions read [this post](http://varianceexplained.org/statistics/interpreting-pvalue-histogram/). **Note** these insight are only for the p-value distribution not for the q-values.
...
The previous tests are used to detect rhythms (recall [slide](#analysis-goals)). The standard approach is to follow detection by Harmonic Regression on the timeseries to obtain amplitudes and phases.
...
**Note** `harmonic.regression` give you the relative and *not* the absolute amplitudes.
...
Such timeseries are statistically but *not* biologically significant. Therefore, we filter the rhythmic timeseries by ensuring the relative-amplitude is greater than threshold (10%-15%).
...
Step 1. Get organized data into your computer or get public datasets from repositories such as GEO using `GEOquery` package.  
...
**2.** Include all effects in analysis, e.g., different batches, different cultures. Check if they matter before ignoring them.
